...
def main():
    ...
    if os.environ.get("WEBBPSF_PATH", None) is None:
        ...
        else: raise TypeError( "Environment variable WEBBPSF_PATH or CLI " + \
                               "arg -w/--webbpsf-path should be set")
    elif args.webbpsf_path is not None:
        warnings.warn( "Environment variable WEBBPSF_PATH exists," + \
                       "but will be overriden by CLI arg -w/--webbpsf-path",
            RuntimeWarning,)
    ...
    for fnmir in calintfiles:
        ...
        run_extraction( fitsimdir=datadir, 
                        ...
                        mnem="", firstfew=firstfew,
                        usebp=True,  # True means do not fit bad pixels when fitting fringes
                        verbose=verbose,)
    ...
    print("\nNext, we calibrate the raw oifits observables... " + \
          "get back list of fullpath calibrated oifits files")
    
    cfnlist, mnmlist = calibrate_pair(os.path.join(datadir,oifdir))
...
def candid_binary_extraction(calib_oifits, mnemonic):
    ...
    fit1 = amical.candid_grid(calib_oifits, **param_candid, diam=0, doNotFit=['diam*'],
                              save=True, outputfile=outputfile)
