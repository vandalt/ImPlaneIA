...
def simulate_data(rot_deg=0.0,psf_offsets_det = (0.0,0.0),phi_waves = np.zeros((7,))):
    ...
    if (fov%2) == 0:
       _psf_offsets_det = (0.5+psf_offsets_det[0],0.5+psf_offsets_det[1])
    else:
       _psf_offsets_det = (0.0+psf_offsets_det[0],0.0+psf_offsets_det[1])
    ...
    #Temporarily overwrite with smaller pistons measured from CV3 data
    #pistons = np.array([5.25e-13, -2.054e-13, -4.300e-14, -2.38e-13,6.21e-15, -2.036e-13,  1.60e-13])
    ...
    jw.simulate(fov=fov, bandpass=bandpass, over=oversample,psf_offset=_psf_offsets_det )
...
def analyze_data(test_tar, affine2d = None, set_center=(0.0,0.0), rotsearch_d = None, set_pistons = None):
    ...
    if (dim%2) == 0:
        set_center = (0.5+set_center[0],0.5+set_center[1])
    else:
        set_center = (0.0+set_center[0],0.0+set_center[1])
...
if __name__ == "__main__":
    ...
    #affine2d_in,psf_offsets_in,pistons_in = simulate_data(rot_deg=rot,psf_offsets_det= (0.0,0.0), phi_waves=np.random.normal(0,1.0, 7)/14.0)
    affine2d_in,psf_offsets_in,pistons_in = simulate_data(rot_deg=rot,psf_offsets_det= (0.48,0.0), phi_waves=np.random.normal(0,1.0, 7)/14.0)
    ...
    for df in datafiles:
        ...
        OR
        affine object (aff) created using user supplied rotation and user supplied psf_offset
        analyze_data(df,affine2d = aff, set_center=(0.48,0.0), rotsearch_d = None, set_pistons = None)
        """

        _aff, _psf_offset, _pistons = analyze_data(df,affine2d = None, set_center=(0.0,0.0), rotsearch_d = _rotsearch_d, set_pistons = None)
        #_aff, _psf_offset, _pistons = analyze_data(df,affine2d = aff, set_center=(0.48,0.0), rotsearch_d = None, set_pistons = None)
